...
export default async function GET() {
    ...
    let firstTime = new Date(new Date(time).setHours(subways?.time?.FIRST_SUBWAY_TIME?.split(':')[0] || 23, subways?.time?.FIRST_SUBWAY_TIME?.split(':')[1] || 20, 0, 0)).getTime();
    ...
    if (lastTime < new Date(time).getTime() - subways?.time?.INTERVAL.substring(0, 1) * 60 *  60 * 1000) {
