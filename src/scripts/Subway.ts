...
export default async function GET() {
    ...
    currentIndex = currentIndex == 0 ? 1 : currentIndex == current.length - 1 ? current.length - 2 : currentIndex;
    // console.log({
    //     subways: current,
    ...
    subway.content = `欢迎乘坐深圳地铁${subways?.line || '5'}号线，本次列车开往${subways?.destination || '黄贝岭'}方向，下一站${subways?.nextStation?.name || '深圳北站'}，请要下车的乘客带好您的行李物品准备下车；文明出行，礼让为先`;
