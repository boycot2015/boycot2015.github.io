...
const _loadingInit = (status: boolean, opacity: number = 0.6, color: string = "#000000"): void => {
    // 形
    const styleTagsWithvhLoading = Array.from(document.querySelectorAll("style")).filter((i: any) => i.textContent.includes("byt-loading"));
    // 无形则有形
    if (styleTagsWithvhLoading.length < 1) {
      // eslint-disable-next-line prettier/prettier
      const vhLoadingStyle = `body>.byt-loading{position:fixed;left:0;top:0;box-sizing:border-box;width:100vw;height:100vh;display:flex;align-items:center;justify-content:center;transition:all .166s;z-index:999;pointer-events:none;opacity:0;overflow:hidden}body>.byt-loading.byt-loading-active{z-index:6666666666666666;pointer-events:auto;opacity:1}body>.byt-loading.byt-loading-active>.byt-loading-span-1,body>.byt-loading.byt-loading-active>.byt-loading-span-2,body>.byt-loading.byt-loading-active>.byt-loading-span-3{position:relative;width:5px;height:5px;margin:3px;border-radius:5px}body>.byt-loading.byt-loading-active>.byt-loading-span-1{-webkit-animation:scale .8s ease infinite;animation:vh_loading_animation .8s ease infinite}body>.byt-loading.byt-loading-active>.byt-loading-span-2{-webkit-animation:scale .8s ease .2s infinite;animation:vh_loading_animation .8s ease .2s infinite}body>.byt-loading.byt-loading-active>.byt-loading-span-3{-webkit-animation:scale .8s ease .4s infinite;animation:vh_loading_animation .8s ease .4s infinite}@-webkit-keyframes vh_loading_animation{50%{height:25px}0%{height:5px}}@keyframes vh_loading_animation{50%{height:25px}0%{height:5px}}`;
    ...
    // 无则有
    let vhLoadingFillDom: any = document.querySelector(".byt-loading");
    let vhLoadingDom_1: any = document.querySelector(".byt-loading-span-1");
    let vhLoadingDom_2: any = document.querySelector(".byt-loading-span-2");
    let vhLoadingDom_3: any = document.querySelector(".byt-loading-span-3");
    if (!vhLoadingFillDom) {
      vhLoadingFillDom = document.createElement("section");
      vhLoadingFillDom.classList.add("byt-loading");
    ...
    // 存在即替换 终有无
    status ? vhLoadingFillDom.classList.add("byt-loading-active") : vhLoadingFillDom.classList.remove("byt-loading-active");
  };
